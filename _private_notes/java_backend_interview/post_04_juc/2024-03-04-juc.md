...
categories: [java_backend_interview, juc]
tags: [juc]
toc: true
math: true
pin: false
...
### 4. 通过 ThreadPoolExecutor 创建线程池（生产环境推荐）

在生产环境中，我们强烈建议直接使用 `ThreadPoolExecutor` 来创建线程池。这种方式可以让我们明确线程池的运行规则，规避资源耗尽的风险。

**核心原理（7 大参数）：**

`ThreadPoolExecutor` 的构造函数包含 7 个关键参数，理解它们是掌握线程池的关键：
...
6.  **threadFactory**：线程工厂，用于创建线程（可以自定义线程名）。
7.  **handler**：拒绝策略，当队列满且线程数达到最大时，如何拒绝新任务。
...
/**
 * 方式四：通过 ThreadPoolExecutor 手动创建线程池
 */
public class ThreadPoolExecutorExample {
    ...
    public static void main(String[] args) {
        ...
        // 使用有界队列，避免 OOM (Out Of Memory)
        BlockingQueue<Runnable> workQueue = new ArrayBlockingQueue<>(2);

        // 自定义线程工厂 (可选，主要为了给线程起个有意义的名字，方便排查问题)
        ThreadFactory threadFactory = new ThreadFactory() {
            private final AtomicInteger count = new AtomicInteger(1);
            @Override
            public Thread newThread(Runnable r) {
                return new Thread(r, "My-Pool-Thread-" + count.getAndIncrement());
            }
        };

        // 拒绝策略：当队列满了且线程达到最大值时，抛出异常 (默认策略)
        ...
        try {
            // 3. 提交任务测试
            // 模拟提交 10 个任务，观察执行情况
            // 预期：2个核心在跑，2个进队列，3个救急线程启动，剩下3个任务会被拒绝策略处理
            for (int i = 1; i <= 10; i++) {
                int taskId = i;
                try {
                    executor.execute(() -> {
                        System.out.println("任务 " + taskId + " 正在运行，执行线程：" + Thread.currentThread().getName());
                        try {
...
1.  如果当前运行的线程数 < `corePoolSize`，则**创建新线程**执行任务。
2.  如果当前运行的线程数 >= `corePoolSize`，则将任务加入 **`workQueue`**。
3.  如果 `workQueue` 已满，但运行线程数 < `maximumPoolSize`，则创建 **非核心线程（救急线程）** 执行任务。
4.  如果队列满了，且运行线程数也达到了 `maximumPoolSize`，则触发 **`handler` (拒绝策略)**。

**为什么比 `Executors` 更好？**

- **避免 OOM**：`Executors.newFixedThreadPool` 使用的是 `LinkedBlockingQueue`（无界队列），如果任务处理不过来，队列会无限增长，最终导致内存溢出。而手动创建可以使用 `ArrayBlockingQueue`（有界队列）。
- **控制资源**：`Executors.newCachedThreadPool` 的最大线程数是 `Integer.MAX_VALUE`，如果任务并发很高，会创建无限多的线程，导致 CPU 100% 或死机。手动创建可以严格限制 `maximumPoolSize`。
