...
categories: [Daily Coding, Coding Tips, 判空]
tags: [判空]
toc: true
math: true
pin: false
...
- **推荐（工具类）：**
  使用 **Apache Commons Collections** 、 **Apache Commons Lang** 或 **Spring Framework** 自带的工具：
...
- **建议：**
  ...
  在**返回值**层面，**永远不要返回 null 的集合或数组**。如果查询无结果，请返回 `Collections.emptyList()` 、 `Collections.emptyMap()` 或 `new int[0]`。
...
**记住一点：**"Null sucks." —— 尽量减少 Null 的传递，如果必须传递，请用 Optional 或注解显式声明。
