...
这个问题的核心是需要为 **每个** 学生和 **每个** 科目的组合生成一个计数，即使某个学生从未参加过某个科目的考试。这可以通过
`CROSS JOIN` 生成所有可能的组合，然后 `LEFT JOIN` 考试记录来实现。
...
1. **`FROM Students st CROSS JOIN Subjects su`**:
   ...
   - `CROSS JOIN` 会创建 `Students` 表和 `Subjects` 表的笛卡尔积。这意味着，它会为每个学生和每个科目生成一个组合行。这个步骤是关键，因为它确保了我们的结果中包含了所有可能的（学生,
     科目）对。
...
2. **`LEFT JOIN Examinations ex ON st.student_id = ex.student_id AND su.subject_name = ex.subject_name`**:
   ...
   - 如果一个学生 **没有** 参加某个科目的考试，连接会失败，来自 `Examinations` 表 (`ex`) 的列（特别是 `ex.subject_name`）将为
     `NULL`。
