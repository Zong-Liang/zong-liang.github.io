...
您需要将 `Project` 表和 `Employee` 表连接起来，然后按 `project_id` 分组，并计算每个组的 `experience_years` 的平均值。
...
1. **`FROM Project p JOIN Employee e ON p.employee_id = e.employee_id`**:
    ...
    * 连接条件 `ON p.employee_id = e.employee_id` 将每个项目中的员工ID与员工表中的详细信息（特别是 `experience_years`
      ）匹配起来。
