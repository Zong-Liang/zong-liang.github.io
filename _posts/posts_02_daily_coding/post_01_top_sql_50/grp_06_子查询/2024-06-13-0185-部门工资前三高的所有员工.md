...
2. **`DENSE_RANK() OVER (PARTITION BY e.departmentId ORDER BY e.salary DESC) AS salary_rank`**: 这是窗口函数的核心部分。
   ...
   - `DENSE_RANK()`: 计算每一行的排名。如果薪水相同，则排名也相同，并且后续排名不会跳跃（例如，排名为 1, 2, 2,
     3）。这完全符合题目中“不同工资中排名前三”的要求。
3. **`SELECT ... FROM RankedSalaries rs JOIN Department d ...`**: 将我们创建的排名结果 `RankedSalaries` (别名为 `rs`) 与
   `Department` 表 (别名为 `d`) 进行连接，以便获取部门的名称。
