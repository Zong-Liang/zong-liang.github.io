...
1.  **快指针 (`right`)**：用于遍历当前数组，寻找所有 **不等于** `val` 的元素。
2.  **慢指针 (`left`)**：指向下一个将要被赋值的位置（即新数组的尾部）。

**具体步骤：**

1.  初始化 `left = 0`。
2.  使用 `right` 从 `0` 到 `nums.length - 1` 遍历数组。
3.  当 `nums[right]` 与 `val` **不相等** 时：

    - 将 `nums[right]` 的值复制到 `nums[left]`。
    - 将 `left` 向后移动一位 (`left++`)。

4.  当 `nums[right]` 与 `val` **相等** 时：

    - 直接跳过，不做处理（相当于删除了该元素）。

5.  遍历结束后，`left` 的值即为新数组的长度。
