...
你可以使用 `LEFT JOIN` 从 `Visits` 表连接到 `Transactions` 表，并筛选出那些没有匹配交易的访问。然后，按 `customer_id` 分组并计数。
...
1. **`FROM Visits v LEFT JOIN Transactions t ON v.visit_id = t.visit_id`**:
   ...
   - `LEFT JOIN` 会返回所有 `Visits` 表中的记录。如果一次访问（由 `v.visit_id` 标识）在 `Transactions` 表中没有对应的交易记录，那么来自`t` 表的所有列（如 `t.transaction_id`）在该连接结果行中将为 `NULL`。
