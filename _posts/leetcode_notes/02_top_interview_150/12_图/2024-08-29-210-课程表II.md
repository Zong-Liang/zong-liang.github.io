...
这题和 207 用的是同一套图模型，区别只在输出：207 只要真假，这题要给出一条可行顺序。

建图仍然是 `[a, b]` 记作 `b -> a`，然后跑 Kahn 拓扑排序。队列里始终放当前入度为 0 的课程；每弹出一个课程，就把它写进结果数组，同时把它指向的课程入度减一，新的 0 入度课程继续进队。

流程结束后有两种结果：如果结果数组长度正好是 `numCourses`，说明全图无环，这个数组就是合法修课顺序；如果没填满，说明存在环，返回空数组。

复杂度与 207 相同：时间 `O(N+M)`，空间 `O(N+M)`。

这一题可以当作 207 的自然延伸来记：同样是“入度清零”过程，判断题看处理数量是否够，构造题则把处理顺序顺手保存下来。
